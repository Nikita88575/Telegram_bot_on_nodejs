  ...
  try {
    ...
    if (msg.text == '/start' || msg.text == `/start@${thisBot.username}`) {
      ...
      if (!user) {
        ...
        await bot.sendMessage(msg.chat.id, 'Я тебе запам`ятав❗️', {
      ...
      if (!user) {
        if (referralUser) {
          ...
          await referralUser.update({
            balance: parseFloat(referralUser.balance) + bonus,
          });
          await bot.sendMessage(
            referralUserId,
            `За твоїм запрошенням приєднався(лась) ${user.first_name}❗️\nБонус: ${bonus}💵❗️`
          );
        } else {
          await bot.sendMessage(
            msg.chat.id,
            'Не знайдено користувача з таким id❗️'
          );
        }
      } else {
        if (user.referral_id) {
          await bot.sendMessage(
            msg.chat.id,
            `Я тебе пам\`ятаю, ти приєднався за запрошенням ${referralUser.first_name}❗️`
          );
        } else {
          await bot.sendMessage(msg.chat.id, 'Я тебе пам`ятаю❗️');
        }
    ...
    console.error(
      'Помилка при створенні або оновленні даних користувача:',
      error
    );
    await bot.sendMessage(
      msg.chat.id,
      'Виникла помилка під час створення або оновлення даних користувача❗️'
    );
  }
}
