...
import {
  ...
  transferRights,
  deleteTeam,
  selectAllBlacklisted,
  addToBlacklist,
  updateTeamUser,
  selectFromBlacklist,
  rmFromBlacklist,
} from '../db/team_commands.js';
  ...
  try {
    ...
    if (query.from.id != user_id) {
      await bot.answerCallbackQuery(query.id, {
        text: 'Ця кнопка не для тебе❗️',
        show_alert: true,
      });
      return;
    } else if (!user) {
      await bot.answerCallbackQuery(query.id, {
        text: 'Ти не в команді❗️',
        show_alert: true,
      });
      return;
    }
...
async function transfer_rights(query) {
  try {
    ...
    const options = {
      inline_keyboard: [
        [{ text: 'Закрити', callback_data: `close_menu#${user_id}` }],
      ],
    };
    ...
    if (data[0] == 'transfer_rights') {
      ...
      if (
        userStates[user_id] &&
        userStates[user_id].state === 'waiting_message'
      ) {
...
async function add_to_blacklist(query) {
  try {
    ...
    const getUserID = async (message) => {
      try {
        if (message.from.id == user_id) {
          ...
          const is_blacklisted = await selectFromBlacklist(
            member_id,
            user.team_id
          );
          ...
          if (!member) {
            await bot.sendMessage(
              message.chat.id,
              'Не знайдено такого користувача❗️'
            );
            return;
          } else if (is_blacklisted) {
            await bot.sendMessage(
              message.chat.id,
              'Цей користувач вже у чорному списку❗️'
            );
            return;
          }
    ...
    if (data[0] == 'add_to_blacklist') {
      ...
      if (
        userStates[user_id] &&
        userStates[user_id].state === 'waiting_user_id'
      ) {
      ...
      try {
        if (message.from.id == user_id) {
          ...
          if (!users) {
            await bot.sendMessage(
              message.chat.id,
              'У чорному списку нікого немає❗️'
            );
            return;
          } else if (!member) {
            await bot.sendMessage(
            ...
            await bot.sendMessage(
              message.chat.id,
              'Цей користувач не у чорному списку❗️'
            );
            return;
          }
