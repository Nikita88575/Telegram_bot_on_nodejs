      ...
      if (msg.reply_to_message) {
        ...
        if (isNaN(value) || value <= 0) {
          return await bot.sendMessage(
            msg.chat.id,
            'Не вірний формат передачі❗️\nВикористовуйте: +[сума]'
          );
        } else if (!toUser) {
          return await bot.sendMessage(
            msg.chat.id,
            'Я не знаю цього користувача❗️'
          );
        } else if (parseFloat(user.balance) < value) {
          return await bot.sendMessage(
            msg.chat.id,
            'У вас недостатньо коштів❗️'
          );
        } else if (msg.reply_to_message.from.is_bot) {
          return await bot.sendMessage(
            msg.chat.id,
            `Не можна передавати кошти боту❗️`
          );
        } else if (msg.from.id == msg.reply_to_message.from.id) {
          return await bot.sendMessage(
            msg.chat.id,
            'Не можна передавати кошти самому собі❗️'
          );
        }
        ...
        await bot.sendMessage(
          msg.chat.id,
          `Ти успішно передав ${value}💵 ${toUser.first_name}`,
