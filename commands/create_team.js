...
async function create_team(msg) {
  try {
    ...
    if (
      msg.text == `${'/createteam'}` ||
      msg.text == `/createteam${thisBot.username}`
    ) {
  ...
  try {
    ...
    if (query.from.id != user_id) {
      ...
      const handlerResponse = async (message) => {
        if (message.text.length >= 3) {
          if (message.from.id == user_id) {
            ...
            await user.update({
              balance: parseFloat(user.balance) - parseInt(item.price),
            });
      ...
      if (data.startsWith('create_team')) {
        await bot.sendMessage(query.message.chat.id, 'Введіть назву команди', {
